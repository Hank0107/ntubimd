...
# 參數設定
# 讀取CSV檔案
df = pd.read_csv('C:/Users/ntubimd/website/news/123/ETF關鍵字/ETF名稱1.csv', header=None, encoding='ANSI')
# 將DataFrame轉換為列表
...
for iSearch in range(len(searchList)):
    ...
    for iLink in range(len(df['link'])):
        ...
        try:
            ...
            imgs.append('C:/Users/ntubimd/website/news/static/images/1.png')
            time.sleep(3)
            continue  # 繼續處理下一個連結
        ...
        if content != "unknow domain":
            ...
            # 使用摘要程式進行摘要
            with open('C:/Users/ntubimd/website/news/123/dictionary/stopWord_summar.txt', 'r', encoding='utf8') as f:  # 停用詞庫
...
try:
    with connection.cursor() as cursor:
        ...
        for index, row in stockNews.iterrows():
            # 檢查 newsUrl 是否為 "unknow domain"
            if row['newsUrl'] != "unknow domain":
