...
if 'none' not in soup.text:
    ...
    for row in rowDatas:
        ...
        if detail_response.status_code == 200:
            ...
            for li_item in detail_list_items:
                ...
                if len(divs) >= 2:
                    industryhold = divs[0].text.strip()
                    holding_percentage = divs[1].text.strip()
                    holding_percentage = re.sub(r'%', '', holding_percentage)
                    result.append([code, stock_name, industryhold, holding_percentage])
                    print([code, stock_name, industryhold, holding_percentage])
    ...
    cursor.execute("DELETE FROM industry_hold")

    insert_query = """
    INSERT INTO industry_hold (stock_code, stock_name, industry_holder, percentage)
    VALUES (%s, %s, %s, %s)
    """
    for data in result:
