...
def update_sentiment_counts():
    # 获取所有的 search_key
    cursor.execute("SELECT search_key FROM news_search_key")
    ...
    for search_key_tuple in search_keys:
        ...
        # 更新 News_search_key 的 positive 和 negative 字段
        cursor.execute(
            "UPDATE news_search_key SET positive = %s, negative = %s WHERE search_key = %s",
